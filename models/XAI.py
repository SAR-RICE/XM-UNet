...
class ChannelAttention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x2 = torch.sigmoid(x2)
        x = x1 + x2
        x = self.mamba(x.squeeze(2))
        x = x.unsqueeze(2)
        x = x.view(-1, self.input_channels, 1, 1)
        return x


class TFIExplainer(nn.Module):
    ...
    def forward(self, inputs):
        src_inputs = inputs
        inputs = self.conv(src_inputs)
        inputs = self.act(inputs)

        channel_att_vec = self.ca(inputs)

        x_init = self.dconv5_5(src_inputs)
        ...
        out = src_inputs * F.sigmoid(channel_att_vec * spatial_att)
        importance = F.adaptive_avg_pool2d(F.sigmoid(channel_att_vec * spatial_att), output_size=(1, 1))
        out = self.conv(out)
