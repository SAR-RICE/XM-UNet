...
class Asandglass(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, norm_in=True, has_skip=True, exp_ratio=1.0, norm_layer='bn_2d',
        ...
        self.conv_local2 = ConvNormAct(dim_mid, dim_mid, kernel_size=1, stride=stride, dilation=dilation,
                                      groups=dim_mid, norm_layer='bn_2d', act_layer='silu', inplace=inplace)
        self.se = SE(dim_mid, rd_ratio=se_ratio, act_layer=get_act(act_layer)) if se_ratio > 0.0 else nn.Identity()

        self.proj_drop = nn.Dropout(drop)
        self.proj = ConvNormAct(dim_mid, dim_out, kernel_size=1, norm_layer='none', act_layer='none', inplace=inplace)
        self.proj_dw = ConvNormAct(dim_mid, dim_out, kernel_size=dw_ks, norm_layer='none', act_layer='none', inplace=inplace)
    ...
    def forward(self, x):
        ...
        x = x + self.se(self.conv_local2(x)) if self.has_skip else self.se(self.conv_local2(x))
