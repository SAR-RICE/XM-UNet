...
# Parameters
height = 224 # Enter the image size of the model.
width  = 224 # Enter the image size of the model.
channels = 12 # Number of image channels

train_number = 6478 # Randomly assign the number of images for generating the training set.
val_number = 1619   # Randomly assign the number of images for generating the validation set.
test_number = 3471  # Randomly assign the number of images for generating the test set.
all = int(train_number) + int(val_number) + int(test_number)

############################################################# Prepare your data set #################################################
Tr_list = glob.glob("/home/xmunet/dataset/VOC2007/images"+'/*.png')   # Images storage folder. The image type should be 24-bit png format.
...
for idx in range(len(Tr_list)):
    ...
    # b = Tr_list[idx]
    # b = b[len(b)-8: len(b)-4]
    file_name_with_extension = os.path.basename(Tr_list[idx])
    b, extension = os.path.splitext(file_name_with_extension)
    add = ("/home/xmunet/dataset/VOC2007/masks/" + b +'.png')  # Masks storage folder. The Mask type should be a black and white image of an 8-bit png (0 pixels for the background and 255 pixels for the target).
    img2 = sc.imread(add)
